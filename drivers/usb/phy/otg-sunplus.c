		...
		if (val == SP_OTG_STATE_A_HOST) {
			if (find_child == false) {
				...
				if ((IS_ENABLED(CONFIG_USB_OTG_WHITELIST)
							&& hcd->tpl_support && targeted) ||
					(!IS_ENABLED(CONFIG_USB_OTG_WHITELIST)
							|| !hcd->tpl_support)) {
					if (IS_ENABLED(CONFIG_USB_OTG_WHITELIST)
								&& hcd->tpl_support && targeted)
...
static void sp_otg_work(struct work_struct *work)
{
	...
	otg_host = (struct sp_otg *)container_of(work, struct sp_otg, work);
...
static void otg_hw_init(struct sp_otg *otg_host)
{
	...
	if (val & ID_PIN)
		writel(~OTG_SIM & (OTG_SRP | OTG_20), &otg_host->regs_otg->mode_select);
	else
		writel(~OTG_SIM & (OTG_ADP | OTG_SRP | OTG_20), &otg_host->regs_otg->mode_select);
}
...
int sp_notifier_call(struct notifier_block *self, unsigned long action, void *dev)
{
	...
	if ((action == USB_DEVICE_REMOVE) && (otg_host->otg.otg->state != OTG_STATE_A_PERIPHERAL)) {
...
static irqreturn_t otg_irq(int irq, void *dev_priv)
{
	...
	if (int_status & ADP_CHANGE_IF) {
		otg_debug("ADP_CHANGE_IF %d %x %x\n", irq, readl(&otg_host->regs_otg->otg_debug_reg),
...
int sp_otg_probe(struct platform_device *dev)
{
	...
	if (!otg_host->otg.otg) {
		kfree(otg_host);
		pr_notice("Alloc mem for otg fail\n");
		return -ENOMEM;
	}
	...
	otg_debug("@@@ otg reg %x %d irq %d %x\n", res_mem->start, resource_size(res_mem),
						otg_host->irq, readl(&otg_host->regs_otg->otg_int_st));
	...
	otg_host->regs_moon4 = (struct sp_regs_moon4 *)ioremap(res_mem->start, resource_size(res_mem));
	...
	if (request_irq(otg_host->irq, otg_irq, IRQF_SHARED, DRIVER_NAME, otg_host) != 0) {
		pr_notice("OTG: Request irq fail\n");
		goto err_ioumap;
	}

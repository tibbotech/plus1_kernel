...
static uint32_t check_trb_status(struct trb_data *t_trb)
{
	...
	case TRANS_ERR:
		UDC_LOGE("trans err,%p,%x,%x,%x,%x\n", t_trb,
			t_trb->entry0, t_trb->entry1, t_trb->entry2, t_trb->entry3);
		ret = TRANS_ERR;
		break;
	case TRB_ERR:
		UDC_LOGE("trb err\n");
...
static void check_event_ring(struct sp_udc *udc)
{
	...
	while (1) {
		...
		if (DEV_EVENT_TRB ==  hal_udc_check_trb_type(event_ring_dq)) {
...
int32_t hal_udc_init(struct sp_udc *udc)
{
	...
	udc->event_ring = (struct udc_ring *)udc_malloc_align(udc,
		&udc->event_ring_pa, udc->event_ring_seg_total * sizeof(struct udc_ring), ALIGN_64_BYTE, GFP_DMA);
	if (!udc->event_ring) {
	...
	for (seg_count = 0; seg_count < udc->event_ring_seg_total; seg_count++) {
		...
		UDC_LOGD("Event_ring[%d]:%px,%llx -> %px,%llx\n", seg_count,
			udc->event_ring[seg_count].trb_va, udc->event_ring[seg_count].trb_pa, tmp_ring->trb_va, tmp_ring->trb_pa);
	}
	...
	case USB_REQ_SET_FEATURE:
		...
		if (ctrl->bRequestType == (USB_DIR_OUT | USB_RECIP_DEVICE)) {
			...
			if (w_value == USB_DEVICE_TEST_MODE) {
				...
				case TEST_J:
					UDC_LOGL("TEST_J\n");
					udc->usb_test_mode = TEST_J;
					value = 0;
					break;
				case TEST_K:
					UDC_LOGL("TEST_K\n");
					udc->usb_test_mode = TEST_K;
					value = 0;
					break;
				case TEST_SE0_NAK:
					UDC_LOGL("TEST_SE0_NAK\n");
					udc->usb_test_mode = TEST_SE0_NAK;
					value = 0;
					break;
				case TEST_PACKET:
					UDC_LOGL("TEST_PACKET\n");
					udc->usb_test_mode = TEST_PACKET;
					value = 0;
					break;
				case TEST_FORCE_EN:
					UDC_LOGL("TEST_FORCE_ENABLE\n");
					udc->usb_test_mode = TEST_FORCE_EN;
					break;
				default:
					UDC_LOGL("Unsupport teset\n");
				}
...
static void cfg_udc_ep(struct sp_udc *udc)
{
	...
	udc->ep_data[3].bEndpointAddress = USB_DIR_OUT | EP3;
	udc->ep_data[3].bmAttributes = USB_ENDPOINT_XFER_BULK;
}
...
static int sp_udc_queue(struct usb_ep *_ep, struct usb_request *_req, gfp_t gfp_flags)
{
	...
	ret = hal_udc_endpoint_transfer(udc, req, ep->bEndpointAddress, _req->buf, _req->dma, _req->length, _req->zero);
	UDC_LOGD("%s: ep:%x len %d, req:%px,trb:%px\n", __func__, ep->bEndpointAddress, _req->length, req, req->transfer_trb);
	if (ret) {
...
static int sp_udc_probe(struct platform_device *pdev)
{
	...
	if (res_mem) {
		moon5_reg = devm_ioremap(&pdev->dev, res_mem->start, resource_size(res_mem));
		if (IS_ERR(moon5_reg)) {
			retval = -ENOMEM;
			goto err_map;
		}

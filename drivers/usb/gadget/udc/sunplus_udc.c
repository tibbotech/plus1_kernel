					...
					struct usb_ctrlrequest *crq, u32 ep0csr, int cf)
{
	...
	// printk(KERN_INFO "bRequestType = %02xh, bRequest = %02xh, wValue = %04xh, wIndex = %04xh, wLength = %04xh, dev->ep0state=%d\n",
	// 	  crq->bRequestType, crq->bRequest, crq->wValue, crq->wIndex, crq->wLength, dev->ep0state);
...
#ifdef USE_DMA
static int sp_udc_bulkout_dma(struct sp_ep *ep,
				       struct sp_request *req)
{
	u8 *buf;
	int actual_length = 0;
...
static int sp_udc_bulkout(struct sp_ep *ep,
				       struct sp_request *req)
{
	u8 *buf;
	u32 count;
...
static int sp_udc_bulkin_pio(struct sp_ep *ep, struct sp_request *req)
{
	u32 count, w_count;
	int is_pingbuf;
	int is_last;
						...
						int is_pingbuf)
{
	...
	DEBUG_DBG(">>> %s\n", __FUNCTION__);
	DEBUG_DBG("UDEPBFS=%xh buf=%xh length=%d actual=%d\n", udc_read(UDEPBFS), req->req.buf, req->req.length, req->req.actual);
	buf = req->req.buf + req->req.actual;
	{
	...
	DEBUG_DBG("ep11 fifo UDEPBFS=%x", udc_read(UDEPBFS));
	DEBUG_DBG("req.length=%d req.actual=%d count=%d is_last=%d\n",
	...
	if (count < ep->ep.maxpacket) {
		...
		if (count != avail) {
			...
			avail -= count;
			printk(KERN_ERR "[%s:%d], overflow [%d]\n", __FUNCTION__,
				  __LINE__, avail);
		}
...
_TX_BULK_IN_DATA:
	...
	udc_write(udc_read(UDEP12C) | SET_EP_IVLD, UDEP12C);
	req->req.actual += count;
	DEBUG_DBG("ep1 req->req.buf=%p count=%d req->req.length=%xh\n", req->req.buf, count, req->req.length);
...
_RX_BULKOUT_DATA_AGAIN:
	...
	if (req->req.actual == 0
		...
		DEBUG_DBG("why len=%d act=%d count=%d\n", nth_len, req->req.actual, sp_udc_get_ep_fifo_count(is_pingbuf, UDEPBPIC));
		return 0;
	}
	udc_write(udc_read(UDNBIE) | EP11O_IF, UDNBIE);
	DEBUG_DBG("<<< %s ...\n", __FUNCTION__);
...
static void sp_sendto_workqueue(
	struct sp_udc *dev, 
	int n)
{
	spin_lock(&qlock);

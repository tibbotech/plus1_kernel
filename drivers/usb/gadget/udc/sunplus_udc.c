...
static u8 first_usb_device_mode_flag = true;
struct semaphore android_enable_sem =
__SEMAPHORE_INITIALIZER(android_enable_sem, 0);
EXPORT_SYMBOL_GPL(android_enable_sem);
...
static void ep_iso_done(struct sp8388_ep *ep, struct sp8388_request *req,
			int status)
{
...
static void sp8388_udc_done(struct sp8388_ep *ep, struct sp8388_request *req,
			    int status)
{
	unsigned halted = ep->halted;

	if (ep->num == EP1 && sp8388_udc_list_empty(&req->queue, &ep->lock)) {
...
static void sp8388_udc_nuke(struct sp8388_udc *udc, struct sp8388_ep *ep,
			    int status)
{
	/* Sanity check */
	DEBUG_DBG(">>> sp8388_udc_nuke...\n");
...
static void sp8388_reinit_iap(struct sp8388_ep *ep, struct sp8388_request *req,
			      int status)
{
	unsigned halted = ep->halted;
	req->req.status = status;
...
static int sp8388_udc_ep_enable(struct usb_ep *_ep,
...
static int sp8388_udc_ep_disable(struct usb_ep *_ep)
{
	...
	DEBUG_DBG(">>> sp8388_udc_ep_disable...\n");
...
static struct usb_request *sp8388_udc_alloc_request(struct usb_ep *_ep,
						    gfp_t mem_flags)
{
	struct sp8388_request *req;
...
static void sp8388_udc_free_request(struct usb_ep *_ep,
...
static int sp8388_udc_write_ep0_fifo(struct sp8388_ep *ep,
				     struct sp8388_request *req);
static int sp8388_udc_read_ep0_fifo(struct sp8388_ep *ep,
				    struct sp8388_request *req);

static int sp8388_udc_get_status(struct sp8388_udc *dev,
...
static void sp8388_udc_handle_ep0s_idle(struct sp8388_udc *dev,
					struct sp8388_ep *ep,
					struct usb_ctrlrequest *crq, u32 ep0csr)
{
	int len;
	int ret;
	DEBUG_INFO(">>>sp8388_udc_handle_ep0s_idle...\n");
...
static inline int sp8388_udc_read_fifo_packet(int fifo, u8 * buf, int length,
					      int regoffset)
{
	int n = 0;
	int m = 0;
...
static inline int sp8388_udc_write_packet(int fifo, struct sp8388_request *req,
...
static inline int sp8388_udc_read_fifo0_packet(int fifo, u8 * buf, int length,
					       int regoffset)
{
	int n = 0;
	int m = 0;
...
#ifdef EP2_ENABLE
static int sp8388_udc_ep2_bulkout_pio(struct sp8388_ep *ep,
				      struct sp8388_request *req,
				      int is_pingbuf)
{
	u8 *buf;
...
#endif
static int sp8388_udc_ep9_bulkout_pio(struct sp8388_ep *ep,
				      struct sp8388_request *req,
				      int is_pingbuf)
{
	u8 *buf;
...
static int sp8388_udc_ep11_bulkout_pio(struct sp8388_ep *ep,
				       struct sp8388_request *req,
				       int is_pingbuf)
{
	u8 *buf;
...
#ifdef EP1_DMA
static int sp8388_udc_ep1_bulkin_dma(struct sp8388_ep *ep,
				     struct sp8388_request *req)
{
	u8 *buf;
	int dma_xferlen;
...
#endif
static int sp8388_udc_ep12_bulkout_dma(struct sp8388_ep *ep,
				       struct sp8388_request *req);
#ifdef EP2_DMA
static int sp8388_udc_ep2_bulkout_pio_with_dma(struct sp8388_ep *ep,
					       struct sp8388_request *req,
					       int is_pingbuf)
{
	u8 *buf;
...
#endif
static int sp8388_udc_ep11_bulkout_dma(struct sp8388_ep *ep,
				       struct sp8388_request *req);

static int sp8388_udc_ep11_bulkout_pio_with_dma(struct sp8388_ep *ep,
						struct sp8388_request *req,
						int is_pingbuf)
{
	u8 *buf;
...
#ifdef EP1_DMA
static int sp8388_ep1_bulkin_dma(struct sp8388_ep *ep,
				 struct sp8388_request *req)
{
	u32 count;
	int is_last = 0;
...
_TX_BULK_IN_DATA:
	...
	if (is_last) {
	    ...
	    if (!sp8388_udc_get_ep_fifo_count
				       ...
				       struct sp8388_request *req)
{
	...
	dma_flag = 2;
	DEBUG_DBG(">>> sp8388_udc_ep12_bulkout_dma...\n");
...
static int sp8388_ep2_bulkout_dma(struct sp8388_ep *ep,
				  struct sp8388_request *req)
{
	int is_pingbuf = 0;
	int ret = 0;
...
static int sp8388_ep11_bulkout_dma(struct sp8388_ep *ep,
				   struct sp8388_request *req)
{
	int is_pingbuf = 0;
	int ret = 0;
...
_RX_BULKOUT_DATA_AGAIN:
	...
	if (req->req.actual == 0
		...
		DEBUG_DBG("why len=%d act=%d dma_len=%d count=%d\n", nth_len,
			  ...
			  sp8388_udc_get_ep_fifo_count(is_pingbuf, UDEPBPIC));
		return 0;
	}
	udc_write(udc_read(UDNBIE) | EP11O_IF, UDNBIE);
	udc_write(udc_read(UDNBIE) | EP11N_IF, UDNBIE);
...
static int sp8388_ep9_bulkout_not_auto(struct sp8388_ep *ep,
				       struct sp8388_request *req)
{
	int is_pingbuf = 0;
...
_RX_BULKOUT_DATA_AGAIN_9:
	if ((!sp8388_udc_get_ep_fifo_count(0, UDEP9PIC)
	     && !sp8388_udc_get_ep_fifo_count(1, UDEP9PIC))
	...
	if (!sp8388_udc_get_ep_fifo_count(is_pingbuf, UDEP9PIC)
...
static void debug_ep_out(void)
{
	...
	DEBUG_DBG("ep out debug %d %x %x\n", ret,
		  sp8388_udc_get_ep_fifo_count(0, UDEP9PIC),
		  sp8388_udc_get_ep_fifo_count(1, UDEP9PIC));
	up(&ep9_ack_sem);
...
static void sphe8388_udc_ep3_work(struct work_struct *work)
{
	struct sp8388_udc *udc = the_controller;
...
static irqreturn_t sp8388_udc_irq(int irq, void *_dev)
{
	...
	if (irq_en2_flags & SUS_IF) {
		DEBUG_DBG("IRQ:UDC Suspent Event\n");
	...
	if (udc_read(UDNBIF) & SOF_IF) {
		...
		if (!is_ncm && dma_len && !down_trylock(&ep11_sw_sem)) {
			if ((udc_read(UDEPBFS) & 0x22) == 0x20 ||
				(sp8388_udc_get_ep_fifo_count((udc_read(UDEPBPPC) & CURR_BUFF) ? 1 : 0, UDEPBPIC) < 512 &&
				sp8388_udc_get_ep_fifo_count((udc_read(UDEPBPPC) & CURR_BUFF) ? 0 : 1, UDEPBPIC) == 512)) {
				if ((udc_read(UDEPBDMACS) & (UDC_FLASH_BUFFER_SIZE - 1)) > 0)	/*verB DMA bug*/
					udc_write(udc_read(UDEPBPPC) | SWITCH_BUFF, UDEPBPPC);
			}
			up(&ep11_sw_sem);
...
static int sp8388_udc_queue(struct usb_ep *_ep, struct usb_request *_req,
			    gfp_t gfp_flags)
{
	struct sp8388_request *req = to_sp8388_req(_req);
	struct sp8388_ep *ep = to_sp8388_ep(_ep);
	...
	if (unlikely(!_req || !_req->complete || !_req->buf
	     || !sp8388_udc_list_empty(&req->queue, &ep->lock))) {
		if (!_req)
			DEBUG_ERR("%s: 1 X X X\n", __func__);
		else {
			DEBUG_ERR("%s: 0 %01d %01d %01d\n", __func__,
				  !_req->complete, !_req->buf,
				  !sp8388_udc_list_empty(&req->queue,
							 &ep->lock));
		}
...
static int sp8388_udc_dequeue(struct usb_ep *_ep, struct usb_request *_req)
{
	...
	DEBUG_INFO("<<< sp8388_udc_dequeue...retval = %d\n", retval);
	return retval;
}
...
static int sp8388_udc_set_selfpowered(struct usb_gadget *_gadget,
				      int is_selfpowered)
{
	return -EOPNOTSUPP;
}
...
static int sp8388_udc_pullup(struct usb_gadget *gadget, int is_on)
{
	if (is_on) {
		...
		/*Force to Disconnect */
		sp8388_udc_nuke(NULL, &memory.ep[9], 0);
		udc_write(udc_read(UDLCSET) | SOFT_DISC, UDLCSET);
	}
	DEBUG_NOTICE("<<< sp8388_udc_pullup...\n");
...
static int sp8388_udc_start(struct usb_gadget *gadget, struct usb_gadget_driver *driver)
{
	struct sp8388_udc *udc = the_controller;
	int ret;

	DEBUG_NOTICE(">>> sp8388_udc_start...\n");
	/* Sanity checks */
	if (!udc)
		return -ENODEV;
	if (udc->driver)
...
static const struct usb_gadget_ops sp8388_ops = {
	...
	.udc_start = sp8388_udc_start,
	.udc_stop = sp8388_udc_stop,
};
...
static void sphe8388_udc_otg_work(struct work_struct *work)
{
	...
	struct sp8388_udc *udc =
	    (struct sp8388_udc *)container_of(work, struct sp8388_udc,
					      work_otg);
	int frame_a;
	int frame_b;
	int otg_id;
...
static int sp8388_udc_probe(struct platform_device *pdev)
{
	...
	if (!udc->qwork_otg) {
		DEBUG_ERR("cannot create workqueue sp8388-udc-otg\n");
		ret = -ENOMEM;
	}
	...
	if (!udc->qwork_ep11) {
		DEBUG_ERR("cannot create workqueue sp8388-udc-ep11\n");
		ret = -ENOMEM;
	}
	INIT_WORK(&udc->work_ep11, sphe8388_udc_ep11_work);

	udc->qwork_ep11_dma =
	    create_singlethread_workqueue("sp8388-udc-ep11_dma");
	...
	if (!udc->qwork_ep1) {
		DEBUG_ERR("cannot create workqueue sp8388-udc-ep1\n");
		ret = -ENOMEM;
	}
	...
	INIT_WORK(&udc->work_ep3, sphe8388_udc_ep3_work);

	udc->qwork_ep9 = create_singlethread_workqueue("sp8388-udc-ep9");
	if (!udc->qwork_ep9) {
		DEBUG_ERR("cannot create workqueue sp8388-udc-ep9\n");
		ret = -ENOMEM;
	}
	INIT_WORK(&udc->work_ep9, sphe8388_udc_ep9_work);
	the_controller = udc;
...
#else
	ret = request_irq(irq, sp8388_udc_irq, IRQF_DISABLED, gadget_name, udc);
#endif
	if (ret != 0) {
	...
	init_ep_spin();
	tasklet_init(&udc->ep1bulkin_task, (pfunc) ep1_handle,
		     (unsigned long)udc);
	sp8388_sem_init();
	...
	DEBUG_ERR("probe sp8388 udc ok %x\n", udc_read(VERSION));
	/*iap debug */
...
err_map:
	DEBUG_INFO("probe sp8388 udc fail\n");
	return ret;
err_mem:
	release_mem_region(rsrc_start, rsrc_len);
error:
	DEBUG_ERR("probe sp8388 udc fail\n");
	return ret;
...
static int sp8388_udc_remove(struct platform_device *pdev)
{
	...
	DEBUG_INFO("sp8388_udc remove ok\n");
	return 0;
}
...
void sp8388_sof_state_polling(unsigned long data)
{
	if (!platform_device_handle_flag
		|| bus_reset_finish_flag){
...
static struct resource sp8388_resource_udc0[] = {
	[0] = {
	       .start = AHB_USBD_BASE0,
	       .end = AHB_USBD_END0,
	       .flags = IORESOURCE_MEM,
	       },
	[1] = {
	       .start = IRQ_USB_DEV_PORT0,
	       .end = IRQ_USB_DEV_PORT0,
	       .flags = IORESOURCE_IRQ,
	       },
};

static struct resource sp8388_resource_udc1[] = {
	[0] = {
	       .start = AHB_USBD_BASE1,
	       .end = AHB_USBD_END1,
	       .flags = IORESOURCE_MEM,
	       },
	[1] = {
	       .start = IRQ_USB_DEV_PORT1,
	       .end = IRQ_USB_DEV_PORT1,
	       .flags = IORESOURCE_IRQ,
	       },
};
struct platform_device sp8388_udc_device0 = {
	.name = "sp8388-udc",
	.id = 1,
	.dev = {
	...
	.num_resources = ARRAY_SIZE(sp8388_resource_udc0),
	.resource = sp8388_resource_udc0,
};

struct platform_device sp8388_udc_device1 = {
	.name = "sp8388-udc",
	.id = 1,
	.dev = {
		.dma_mask = &sp8388_udc_dma_mask,
	...
	.num_resources = ARRAY_SIZE(sp8388_resource_udc1),
	.resource = sp8388_resource_udc1,
};
...
static void udc_work(void)
{
	char *configured[2] = { "USB_STATE=CONNECT_FAIL", NULL };
	if (accessory_port_id == 0){
		kobject_uevent_env(&sp8388_udc_device0.dev.kobj, KOBJ_CHANGE,
			 	configured);
	}else{
		kobject_uevent_env(&sp8388_udc_device1.dev.kobj, KOBJ_CHANGE,
			   	configured);
	}
}
...
void usb_switch(int device)
{
	...
	if (device) {
		...
		if (is_config) {
			is_config = 0;
			sp8388_udc_nuke(memory.ep[8].dev, &memory.ep[8],
					-ESHUTDOWN);
			/*sp8388_reinit_iap(&memory.ep[9],
						list_entry((&memory.ep[9])->queue.next,
...
static int __init udc_init(void)
{
	...
	sof_polling_timer.function = sp8388_sof_state_polling;

...
static int sp_udc_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		pr_err("cannot get irq %i, err %d", irq_num, ret);
		ret = -EBUSY;
		goto err_map;
	}
...
{
	/* control rx signal */
	udc_write(udc_read(UEP12DMACS) | RX_STEP7, UEP12DMACS);
	DEBUG_DBG("%s UEP12DMACS: %x", __func__, udc_read(UEP12DMACS));
}

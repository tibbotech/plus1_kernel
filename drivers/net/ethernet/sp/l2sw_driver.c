...
static inline void rx_skb(struct l2sw_mac *mac,
			  struct sk_buff *skb, volatile struct mac_desc *desc)
{
...
static inline void interrupt_finish(struct l2sw_mac *mac)
{
	...
	//reg_control(REG_WRITE, MAC_SW_INT_MASK0, 0x00000000);
}
...
static void rx_do_tasklet(unsigned long data)
{
	...
	//reg_control(REG_WRITE, MAC_SW_INT_STATUS0, (MAC_INT_RX) & mac->int_status);
...
static void tx_do_tasklet(unsigned long data)
{
	...
	tx_interrupt(mac);
	reg_control(REG_WRITE, MAC_SW_INT_STATUS0, (MAC_INT_TX) & mac->int_status);
	wmb();
	interrupt_finish(mac);
}
#endif
...
static irqreturn_t ethernet_interrupt(int irq, void *dev_id)
{
	...
	*/
	reg_control(REG_WRITE, MAC_SW_INT_MASK0, 0xffffffff); /* mask interrupt */
	status =  reg_control(REG_READ, MAC_SW_INT_STATUS0, 0);
	...
	reg_control(REG_WRITE, MAC_SW_INT_STATUS0, status);
	wmb();

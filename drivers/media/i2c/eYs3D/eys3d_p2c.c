...
static int p2c_wait_reg_for_target(struct i2c_client* client, uint16_t reg, uint8_t val, uint8_t target)
{
...
retry_new_cmd:
    ...
    /* For Grape Command Trigger Begin */
    if (val == EYS3D_API_CRF_DEV_RDY_ENTER) {
        ret = eys3d_wr_reg(fd, EYS3D_API_CMD_TRIG_INT_REG, 0x11);
        I2C_ERR_HANDLE(ret, ret_line);
        
        ret = eys3d_rd_reg(fd, EYS3D_API_CMD_TRIG_INT_REG, buf);
        I2C_ERR_HANDLE(ret, ret_line);
        
        EYS3D_I2C_DBG_PRT("Grape Command Trigger [0x%04X] = [0x11] -> [0x%02X](%d)\n", EYS3D_API_CMD_TRIG_INT_REG, buf[0], ret);
    }
    /* For Grape Command Trigger End */
...
static int p2c_chk_vid_stat(struct i2c_client* client, uint8_t *val)
{
    struct i2c_client *fd = client;
    int ret = EYS3D_API_RET_OK;
    int ret_line = 0;
    uint8_t cmd = EYS3D_API_CMD_TP_CHK_VID_STAT_RD;
    struct v4l2_subdev *sd = i2c_get_clientdata(client);
    struct eys3d *driv = sd_to_eys3d(sd);
    struct eys3d_api_data *priv = driv->data;
    ...
    ret = p2c_wait_reg_for_target(fd, EYS3D_API_CMD_TRIG_REG, EYS3D_API_CRF_DEV_RDY_ENTER, EYS3D_API_CRF_DEV_RDY_LEAVE);
    ...
    eys3d_sleep(EYS3D_API_SLEEP_PARM);
    eys3d_wr_reg(fd, EYS3D_API_CMD_ADR_REG, EYS3D_API_CMD_REQ & 0xFF);
    eys3d_wr_reg(fd, EYS3D_API_CMD_DAT_REG, cmd);                           // [0xDC01] [0x81]

    eys3d_wr_reg(fd, EYS3D_API_CMD_ADR_REG, EYS3D_API_CMD_OPT & 0xFF);
    eys3d_wr_reg(fd, EYS3D_API_CMD_DAT_REG, 0x00);                          // [0xDC02] [0x00]

    ret = p2c_wait_reg_for_target(fd, EYS3D_API_CMD_TRIG_REG, EYS3D_API_CRF_CMD_PRE_ENTER, EYS3D_API_CRF_CMD_PRE_LEAVE);
    if (ret != EYS3D_API_RET_OK) {
        ret_line = __LINE__;
        goto exit;
    }

    eys3d_wr_reg(fd, EYS3D_API_CMD_ADR_REG, EYS3D_API_CMD_DAT & 0xFF);
    eys3d_rd_reg(fd, EYS3D_API_CMD_DAT_REG, val);                           // [0xDC20] [Val]
    EYS3D_API_DBG_PRT("[EYS3D_API_CMD_DAT_REG] [0x%02X]\n", *val);

    ret = p2c_wait_reg_for_target(fd, EYS3D_API_CMD_TRIG_REG, EYS3D_API_CRF_CMD_CLS_ENTER, EYS3D_API_CRF_CMD_CLS_LEAVE);
    if (ret != EYS3D_API_RET_OK) {
        ret_line = __LINE__;
        goto exit;
    }
...
static int p2c_sel_dev(struct eys3d *driv)
{
    int ret = EYS3D_API_RET_OK;
    struct eys3d_api_data *priv = driv->data;
    struct i2c_client     *client = driv->client;

    if ((priv->vc_id == EYS3D_API_VC_ID_0) ||
        (priv->vc_id == EYS3D_API_VC_ID_1)) {
        uint8_t b4_buf = 0, aft_buf = 0;

        EYS3D_ALERT_DBG_PRT("VC[%d] is supported!\n", priv->vc_id);
        ret = p2c_rd_fw_reg(client, EYS3D_API_FW_REG_SEL_DEV, &b4_buf);
        ret = p2c_wr_fw_reg(client, EYS3D_API_FW_REG_SEL_DEV, priv->vc_id + 1);
        ret = p2c_rd_fw_reg(client, EYS3D_API_FW_REG_SEL_DEV, &aft_buf);
        EYS3D_API_DBG_PRT("Select device [0x%02X] [0x%02X -> 0x%02X]\n", EYS3D_API_FW_REG_SEL_DEV, b4_buf, aft_buf);
    } else {
        EYS3D_ERR_DBG_PRT("VC#% is NOT supported!\n", priv->vc_id);
        ret = -EINVAL;
        goto exit;
    }
...
static int p2c_rd_next(struct i2c_client* client, unsigned char *input, int size, bool has_next)
{
    int length = size;
    int i = 0;
    int ret = EYS3D_API_RET_OK;
    ...
    if (has_next) {
        ret = p2c_wait_reg_for_target(fd, EYS3D_API_CMD_TRIG_REG, EYS3D_API_CRF_CMD_NEX_ENTER, EYS3D_API_CRF_CMD_PRE_LEAVE);
        if (ret != EYS3D_API_RET_OK) {
            ret = -EBUSY;
            goto exit;
        }
...
static int p2c_wr_next(struct i2c_client* client, unsigned char *input, int size, bool has_next, bool first_wr_)
{
    int length = size;
    int i = 0;
    int ret = EYS3D_API_RET_OK;
...
int p2c_get_pid_vid(struct i2c_client* client, uint16_t *pid, uint16_t *vid)
{
    ...
    *vid = 0;
    ...
    /* 0x4e, 0x1e, 0x67, 0x01 */
    ret = p2c_rd_fw_data(client, EYS3D_API_IO_REQ_MEM_TP_FS_TABLE, id, 0, data, 32);
    if (ret == EYS3D_API_RET_OK) {
        val = data[3];
        val = val << 8;
        val |= data[2];
...
int p2c_enable_mipi_output(struct i2c_client* client, bool en)
{
    ...
    if (en) {
#if 0 // Do NOT need to do this on Grape
        ret = p2c_wr_fw_reg(client, 0xF1, 0x10); // MIPI output
#endif
        ENTER_CS
        if (ret == EYS3D_API_RET_OK) {
...
#if 0 // Do NOT need to do this on Grape
        ret = p2c_wr_fw_reg(client, 0xF1, 0x00); // USB output
#endif
        ENTER_CS
        if (ret == EYS3D_API_RET_OK) {
            priv->mipi_out_en = false;
...
int p2c_enable_sensor_if(struct i2c_client* client)
{
    ...
    if (ret == EYS3D_API_RET_OK) {
        uint8_t wr_buf = EYS3D_API_HW_REG_SENSORIF_ENABLE, b4_buf = 0, aft_buf = 0;

        ret = p2c_rd_asic_reg(client, EYS3D_API_HW_REG_SENSORIF, &b4_buf, sizeof(b4_buf));
        ret = p2c_wr_asic_reg(client, EYS3D_API_HW_REG_SENSORIF, &wr_buf, sizeof(wr_buf));
        ret = p2c_rd_asic_reg(client, EYS3D_API_HW_REG_SENSORIF, &aft_buf, sizeof(aft_buf));
        EYS3D_API_DBG_PRT("Enable Sensor IF [0x%02X] [0x%02X -> 0x%02X]\n", EYS3D_API_HW_REG_SENSORIF, b4_buf, aft_buf);
    }
...
int p2c_video_switch(struct i2c_client* client, uint32_t mode_index)
{
    ...
    /* For Grape Select Device Begin */
    ret = p2c_sel_dev(driv);
    if (ret != EYS3D_API_RET_OK) {
        EYS3D_ERR_DBG_PRT("Failed to select device\n");
        ret_line = __LINE__;
        goto exit;
    }
    /* For Grape Select Device End */
    ...
    /* For Grape Check Video State Begin */
    EYS3D_ALERT_DBG_PRT("Check Video State\n");
    {
        unsigned int try_cnt = 0;
        uint8_t state = 0;
        
        while (1) {
            ret = p2c_chk_vid_stat(client, &state);
            if (try_cnt == EYS3D_API_CHK_VID_STAT_TRY_MAX_CNT) {
                try_cnt = 0;
                ret = -EBUSY;
                ret_line = __LINE__;
                goto exit;
            }
            if ((state & EYS3D_API_CHK_VID_STAT_BUSY_BIT) == EYS3D_API_CHK_VID_STAT_NO_BUSY) {
                ret = EYS3D_API_RET_OK;
                break;
            }
            else eys3d_sleep(EYS3D_API_SLEEP_PARM);
    ...
    /* For Grape Check Video State End */
...
int p2c_video_close(struct i2c_client* client)
{
    ...
    ret = p2c_sel_dev(driv);
    if (ret != EYS3D_API_RET_OK) {
        EYS3D_ERR_DBG_PRT("Failed to Grape select device\n");
        ret_line = __LINE__;
        goto exit;
    }
    /* For Grape Select Device End */
...
exit:
    EYS3D_API_DBG_PRT("Leave[%d, %d, 0x%X]\n", ret, ret_line, *val);
    LEAVE_CS
...
int p2c_rd_asic_reg(struct i2c_client* client, uint16_t reg, uint8_t *data, uint16_t len)
{
    ...
    if (len <= wr_buf) {
        ...
        for (i = 0; i < count; i++){
            ...
            ret = p2c_rd_next(fd, &data[i * wr_buf], wr_buf, has_next);
            if (ret != EYS3D_API_RET_OK) {
                ret_line = __LINE__;
                goto exit;
            }
        ...
        if (diff != 0) {
            ret = p2c_rd_next(fd, &data[i * wr_buf], diff, false);
        }
    }
...
int p2c_wr_asic_reg(struct i2c_client* client, uint16_t reg, uint8_t *data, uint16_t len)
{
    ...
    if (len <= wr_buf) {
        ...
        for (i = 0; i < count; i++){
            ...
            ret = p2c_wr_next(fd, &data[i * wr_buf], wr_buf, has_next, first_w);
            if (ret != EYS3D_API_RET_OK) {
                ret_line = __LINE__;
                goto exit;
            }
        ...
        if (diff != 0) {
            ret = p2c_wr_next(fd, &data[i * wr_buf], diff, false, first_w);
        }
    }
...
static int p2c_property_bar_val_mask(uint8_t id, uint8_t ctrsel, uint32_t *mask)
{
    int i = 0;
    int num = 0;
...
int p2c_rd_prop_bar(struct i2c_client* client, uint8_t id, uint8_t type, uint8_t ctrsel, uint32_t *val)
{
    ...
    (void)priv;
    (void)mask;
    (void)p2c_property_bar_val_mask;
    ...
    *val = (*val & ~0xFF000000) | (tmp32 << 24);
    ...
    if (p2c_property_bar_val_mask(id, ctrsel, &mask) == 0) {
        *val = *val & mask;
    }
...
int p2c_rd_fw_data(struct i2c_client* client, uint8_t type, uint8_t id, uint32_t offset, uint8_t* data, uint16_t len)
{
    ...
    if (len <= EYS3D_API_MAX_FS_BUF_SZ) {
        ret = p2c_rd_fd(client, type, id, offset, ptr, len);
        if (ret != EYS3D_API_RET_OK) {
            goto exit;
        }
    } else {
        ...
        while (nOffset < len) {
            ...
            ret = p2c_rd_fd(client, type, id, nOffset, (ptr + nOffset), nLength);
            if (ret != EYS3D_API_RET_OK) {
                goto exit;
            }
            nOffset += nLength;
...
int p2c_wr_fw_data(struct i2c_client* client, uint8_t type, uint8_t id, uint32_t offset, uint8_t* data, uint16_t len)
{
    ...
    if (len <= EYS3D_API_MAX_FS_BUF_SZ) {
        ret = p2c_wr_fd(client, type, id, offset, ptr, len);
        if (ret != EYS3D_API_RET_OK) {
            goto exit;
        }
    } else {
        ...
        while (nOffset < len) {
            ...
            ret = p2c_wr_fd(client, type, id, nOffset, (ptr + nOffset), nLength);
            if (ret != EYS3D_API_RET_OK) {
                goto exit;
            }
            nOffset += nLength;
...
int eys3d_p2c_start_stream(struct eys3d *driv)
{
    ...
    if (priv->is_first_set_video_mode) {
        ret = p2c_video_switch(client, priv->cur_video_mode_index);
        if (ret != EYS3D_API_RET_OK) {

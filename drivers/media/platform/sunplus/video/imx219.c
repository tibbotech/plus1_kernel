...
static const struct imx219_mode_info imx219_mode_data[IMX219_NUM_MODES] = {
...
static int imx219_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	DBG_INFO("%s, streaming: %d, pending_mode_change: %d, pending_fmt_change: %d\n",
		 __func__, imx219->streaming, imx219->pending_mode_change,
		 ...
		 int width, int height, bool nearest)
{
	...
	if (fr == IMX219_15_FPS && !(mode->hact <= 3280 && mode->vact <= 2464))
		return NULL;

	/* 1920x1080 (1080P) can operate at max frame rate 48fps */
	if (fr == IMX219_48_FPS && !(mode->hact <= 1920 && mode->vact <= 1080))
		return NULL;
				      ...
				      struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	ret = imx219_try_frame_interval(imx219, &tpf, fie->width, fie->height);
	if (ret < 0)
		return -EINVAL;
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	if (format->which == V4L2_SUBDEV_FORMAT_TRY)
		fmt = v4l2_subdev_get_try_format(&imx219->subdev, cfg, format->pad);
	else
		fmt = &imx219->fmt;
#else
	fmt = &imx219->fmt;
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	ret = imx219_try_fmt_internal(sd, mbus_fmt, imx219->current_fr, &new_mode);
	if (ret)
		goto out;

	DBG_INFO("%s, mbus_fmt->code: 0x%04x, imx219->fmt.code: 0x%04x\n",
		 __func__, mbus_fmt->code, imx219->fmt.code);
	...
	*fmt = *mbus_fmt;
	...
	if ((mbus_fmt->code != imx219->fmt.code) || (orig_fmt.code != imx219->fmt.code))
		imx219->pending_fmt_change = true;

	DBG_INFO("%s, code: 0x%04x, pending_mode_change: %d, pending_fmt_change: %d\n",
		 __func__, imx219->fmt.code, imx219->pending_mode_change,
		 imx219->pending_fmt_change);

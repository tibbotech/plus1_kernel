...
static const struct ov5647_mode_info ov5647_mode_data[OV5647_NUM_MODES] = {
...
static int ov5647_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	DBG_INFO("%s, streaming: %d, pending_mode_change: %d, pending_fmt_change: %d\n",
		 __func__, ov5647->streaming, ov5647->pending_mode_change,
		 ...
		 int width, int height, bool nearest)
{
	...
	if (fr == OV5647_60_FPS && !(mode->hact == 640 && mode->vact == 480))
		return NULL;

	/* 2592x1944 only works at 15fps max */
	if ((mode->hact == 2592 && mode->vact == 1944) && fr > OV5647_15_FPS)
		return NULL;
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	if (format->which == V4L2_SUBDEV_FORMAT_TRY)
		fmt = v4l2_subdev_get_try_format(&ov5647->subdev, cfg, format->pad);
	else
		fmt = &ov5647->fmt;
#else
	fmt = &ov5647->fmt;
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	ret = ov5647_try_fmt_internal(sd, mbus_fmt, ov5647->current_fr, &new_mode);
	if (ret)
		goto out;

	DBG_INFO("%s, mbus_fmt->code: 0x%04x, ov5647->fmt.code: 0x%04x\n",
		 __func__, mbus_fmt->code, ov5647->fmt.code);
	...
	*fmt = *mbus_fmt;
	...
	DBG_INFO("%s, code: 0x%04x, pending_mode_change: %d, pending_fmt_change: %d\n",
		 __func__, ov5647->fmt.code, ov5647->pending_mode_change,
		 ov5647->pending_fmt_change);

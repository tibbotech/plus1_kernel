...
static const struct gc0310_mode_info gc0310_mode_data[GC0310_NUM_MODES] = {
...
static int gc0310_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	DBG_INFO("%s, streaming: %d, pending_mode_change: %d, pending_fmt_change: %d\n",
		 __func__, gc0310->streaming, gc0310->pending_mode_change,
...
static int gc0310_enum_frame_interval(struct v4l2_subdev *sd,
				      struct v4l2_subdev_pad_config *cfg,
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	DBG_INFO("%s, mbus_fmt->code: 0x%04x, gc0310->fmt.code: 0x%04x\n",
		 __func__, mbus_fmt->code, gc0310->fmt.code);
	...
	*fmt = *mbus_fmt;
	...
	DBG_INFO("%s, code: 0x%04x, pending_mode_change: %d, pending_fmt_change: %d\n",
		 __func__, gc0310->fmt.code, gc0310->pending_mode_change,
		 gc0310->pending_fmt_change);

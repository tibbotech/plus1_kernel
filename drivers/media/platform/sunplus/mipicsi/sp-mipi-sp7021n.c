...
/* Register accessors */
static inline u32 csi_readl(struct sp_mipi_device *dev, u32 reg)
{
	return readl(dev->mipicsi_regs + reg);
}

static inline void csi_writel(struct sp_mipi_device *dev, u32 reg, u32 value)
{
	writel(value, dev->mipicsi_regs + reg);
}

static inline u32 iw_readl(struct sp_mipi_device *dev, u32 reg)
{
	return readl(dev->csiiw_regs + reg);
}

static inline void iw_writel(struct sp_mipi_device *dev, u32 reg, u32 value)
{
	writel(value, dev->csiiw_regs + reg);
}
...
static void sp_mipicsi_lane_config(struct sp_mipi_device *mipi)
{
	...
	mix_cfg = csi_readl(mipi, SP_MIPICSI_MIX_CFG);
	ana_cfg2 = csi_readl(mipi, SP_MIPI_ANALOG_CFG2);
	...
	dev_dbg(mipi->dev, "%s, num_lanes: %d\n", __func__, num_lanes);
	...
	dev_dbg(mipi->dev, "%s, %d: mix_cfg: %08x, ana_cfg2: %08x\n",
		 __func__, __LINE__, mix_cfg, ana_cfg2);

	csi_writel(mipi, SP_MIPICSI_MIX_CFG, mix_cfg);
	csi_writel(mipi, SP_MIPI_ANALOG_CFG2, ana_cfg2);
}
...
static void sp_mipicsi_dt_config(struct sp_mipi_device *mipi)
{
	...
	mix_cfg = csi_readl(mipi, SP_MIPICSI_MIX_CFG);
	sync_word = csi_readl(mipi, SP_MIPICSI_SOF_SOL_SYNCWORD);

	dev_dbg(mipi->dev, "%s, mipi->sd_format->sol_sync: %02x\n",
		 __func__, mipi->sd_format->sol_sync);

	switch (sol_sync) {
	default:
	case SYNC_RAW8:		/* 8 bits */
	case SYNC_YUY2:
		set_field(&mix_cfg, 2, 0x3<<16);
		break;

	case SYNC_RAW10:	/* 10 bits */
		set_field(&mix_cfg, 1, 0x3<<16);
		break;
	...
	csi_writel(mipi, SP_MIPICSI_MIX_CFG, mix_cfg);
	csi_writel(mipi, SP_MIPICSI_SOF_SOL_SYNCWORD, sync_word);
}
...
static void sp_mipicsi_reset_s2p_ff(struct sp_mipi_device *mipi)
{
	...
	ana_cfg1 = csi_readl(mipi, SP_MIPI_ANALOG_CFG1);

	/* Reset Serial-to-parallel Flip-flop */
	set_field(&ana_cfg1, 0x1, 0x1<<0);		/* RSTS2P = 1: Reset mode */
	udelay(1);
	csi_writel(mipi, SP_MIPI_ANALOG_CFG1, ana_cfg1);
	udelay(1);
...
static void sp_csiiw_fs_config(struct sp_mipi_device *mipi)
{
	...
	stride = iw_readl(mipi, SP_CSIIW_STRIDE);
	frame_size = iw_readl(mipi, SP_CSIIW_FRAME_SIZE);
	...
	set_field(&frame_size, pix->width, 0x1fff<<0);
	...
	iw_writel(mipi, SP_CSIIW_STRIDE, stride);
	iw_writel(mipi, SP_CSIIW_FRAME_SIZE, frame_size);
}

static void sp_csiiw_wr_dma_addr(struct sp_mipi_device *mipi, dma_addr_t addr)
{
	/* Set BASE_ADDR field of csiiw_base_addr */
	iw_writel(mipi, SP_CSIIW_BASE_ADDR, addr);
}
...
static void sp_csiiw_enable(struct sp_mipi_device *mipi)
{
	...
	dev_dbg(mipi->dev, "%s, %d: bpp: %d\n", __func__, __LINE__, mipi->sd_format->bpp);

	/* 
	 * Bit[4] Source data format
	 * 0: 8 bits per pixel (default)
	 * 1: 10 bits per pixel
	 */
	b4 = (mipi->sd_format->bpp == 10)? 1:0;

	config0 = iw_readl(mipi, SP_CSIIW_CONFIG0);
	...
	set_field(&config0,  b4, 0x1<<4);	/* Source data format */
	set_field(&config0, 0x1, 0x1<<0);	/* Enable CSIIW function */
	iw_writel(mipi, SP_CSIIW_CONFIG0, config0);

	dev_dbg(mipi->dev, "%s, %d: config0: 0x%08x\n", __func__, __LINE__, config0);
}
...
static void sp_mipicsi_init(struct sp_mipi_device *mipi)
{
	...
	csi_writel(mipi, SP_MIPICSI_MIX_CFG, val);

	val = 0;
	set_field(&val, SYNC_YUY2, 0xffff<<0);		/* Set SOL_SYNCOWRD */
	csi_writel(mipi, SP_MIPICSI_SOF_SOL_SYNCWORD, val);
	...
	csi_writel(mipi, SP_MIPI_ANALOG_CFG2, val);

	csi_writel(mipi, SP_MIPICSI_ECC_CFG, 0x110);
	csi_writel(mipi, SP_MIPI_ANALOG_CFG1, 0x1000);
	csi_writel(mipi, SP_MIPI_ANALOG_CFG1, 0x1001);
	udelay(1);
	csi_writel(mipi, SP_MIPI_ANALOG_CFG1, 0x1000);
	csi_writel(mipi, SP_MIPICSI_ENABLE, 0x1);	/* Enable MIPICSI */
}

static void sp_csiiw_init(struct sp_mipi_device *mipi)
{
	iw_writel(mipi, SP_CSIIW_LATCH_MODE, 0x1);		/* Enable before base addr setup */
	iw_writel(mipi, SP_CSIIW_STRIDE, 0x500);
	iw_writel(mipi, SP_CSIIW_FRAME_SIZE, 0x3200500);
	iw_writel(mipi, SP_CSIIW_FRAME_BUF, 0x00000100);	/* set offset to trigger DRAM write */
	iw_writel(mipi, SP_CSIIW_CONFIG0, 0x32700);		/* Disable csiiw, fs_irq and fe_irq */
}

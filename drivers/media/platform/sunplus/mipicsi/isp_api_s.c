...
/*
	@ispReset_s
*/
void ispReset_s(struct mipi_isp_info *isp_info)
{
	struct mipi_isp_reg *regs = (struct mipi_isp_reg *)((u64)isp_info->mipi_isp_regs - ISP_BASE_ADDRESS);
...
*/
void FrontInit_s(struct mipi_isp_info *isp_info)
{
	struct mipi_isp_reg *regs = (struct mipi_isp_reg *)((u64)isp_info->mipi_isp_regs - ISP_BASE_ADDRESS);
...
/*
	@cdspSetTable_s
*/
void cdspSetTable_s(struct mipi_isp_info *isp_info)
{
	struct mipi_isp_reg *regs = (struct mipi_isp_reg *)((u64)isp_info->mipi_isp_regs - ISP_BASE_ADDRESS);
	int i;
	...
	for (i = 0; i < 768; i++)
	{
		writeb(SF_LENS_COMP_R_S[i], &(regs->reg[0x2103]));
	}
	//
	...
	for (i = 0; i < 768; i++)
	{
		writeb(SF_LENS_COMP_G_S[i], &(regs->reg[0x2103]));
	}
	//
	...
	for (i = 0; i < 768; i++)
	{
		writeb(SF_LENS_COMP_B_S[i], &(regs->reg[0x2103]));
	}
	//
	...
	for (i = 0; i < 512; i++)
	{
		writeb(SF_POST_GAMMA_R_S[i], &(regs->reg[0x2103]));
	}
	//
	...
	for (i = 0; i < 512; i++)
	{
		writeb(SF_POST_GAMMA_G_S[i], &(regs->reg[0x2103]));
	}
	//
	...
	for (i = 0; i < 512; i++)
	{
		writeb(SF_POST_GAMMA_B_S[i], &(regs->reg[0x2103]));
	}
	//
	...
	for (i = 0; i < 1312; i++)
	{
		writeb(SF_FIXED_PATTERN_NOISE_S[i], &(regs->reg[0x2103]));
	}

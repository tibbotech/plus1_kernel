...
#define REGSET32(r)                                                            \
	{                                                                      \
		__stringify(r), TOP_REG(DL1_RP, DL1_##r) - TOP_REG(0, 0)       \
	}
...
static int ethosn_mailbox_init(struct ethosn_core *core)
{
	...
	mailbox->severity =
		max(min(severity, ETHOSN_LOG_VERBOSE), ETHOSN_LOG_PANIC);
...
static int ethosn_boot_firmware(struct ethosn_core *core)
{
	...
	vtable[1] =
		to_ethosn_addr(core->firmware->iova_addr, &core->firmware_map) +
		1;
 ...
 * Return: Negative error code on error, zero otherwise
 */
int ethosn_set_addr_ext(struct ethosn_core *core, unsigned int stream,
			...
			size_t length)
{
	...
	if (core->mailbox_response->size < (sizeof(*queue) + queue->capacity) ||
	...
	if ((ethosn_queue_get_size(queue) -
	     sizeof(struct ethosn_message_header)) < header->length)
		return 0;
			 ...
			 size_t length)
{
	...
	ret = ethosn_queue_write(queue, (uint8_t *)&header,
				 sizeof(struct ethosn_message_header),
				 &write_pending);
	if (!ret)
		return ret;
			       ...
			       enum ethosn_stream_id stream_id)
{
	...
	dev_dbg(core->dev, "-> Stream=%u. size=0x%x", request.stream_id,
		request.size);
						   ...
						   struct ethosn_big_fw *big_fw)
{
	...
	printk("NPU DL1_UNIT_COUNT: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_MCE_FEATURES);
	printk("NPU DL1_MCE_FEATURES: %x\n",temp);
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_DFC_FEATURES);
	printk("NPU DL1_DFC_FEATURES: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_PLE_FEATURES);
	printk("NPU DL1_PLE_FEATURES: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_WD_FEATURES);
	printk("NPU DL1_WD_FEATURES: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_VECTOR_ENGINE_FEATURES);
	printk("NPU DL1_VECTOR_ENGINE_FEATURES: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_ECOID);
	...
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_GP0);
	printk("NPU DL1_GP0: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_GP1);
	printk("NPU DL1_GP1: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_GP2);
	printk("NPU DL1_GP2: %x\n",temp);
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_GP3);
	printk("NPU DL1_GP3: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_GP4);
	printk("NPU DL1_GP4: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_GP5);
	printk("NPU DL1_GP5: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_GP6);
	printk("NPU DL1_GP6: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_GP7);
	printk("NPU DL1_GP7: %x\n",temp);
#endif
#if 0 //3000~3018//hang up
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_STREAM0_STREAM_SECURITY);
	printk("NPU DL1_STREAM0_STREAM_SECURITY: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_STREAM0_NSAID);
	printk("NPU DL1_STREAM0_NSAID: %x\n",temp);		
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_STREAM0_MMUSID);
	printk("NPU DL1_STREAM0_MMUSID: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_STREAM0_MMUSSID);
	printk("NPU DL1_STREAM0_MMUSSID: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_STREAM0_ATTR_CONTROL);
	printk("NPU DL1_STREAM0_ATTR_CONTROL: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_STREAM0_MEMATTR);
	printk("NPU DL1_STREAM0_MEMATTR: %x\n",temp);
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_STREAM0_ADDRESS_EXTEND);
	printk("NPU DL1_STREAM0_ADDRESS_EXTEND: %x\n",temp);
#endif
#if 0 //0034 0040 00A0//hang up
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_CLRIRQ_EXT);
	printk("NPU DL1_CLRIRQ_EXT: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_SETIRQ_INT);
	printk("NPU DL1_SETIRQ_INT: %x\n",temp);	
	...
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_SYSCTLR1);
	printk("NPU DL1_SYSCTLR1: %x\n",temp);	
	temp = ethosn_read_top_reg(core, DL1_RP, DL1_PWRCTLR);
	printk("NPU DL1_PWRCTLR: %x\n",temp);
#endif
	...
	dev_dbg(core->dev, "NPU reported version %u.%u.%u. FWs in BIG FW: %u\n",
		npu_id.bits.arch_major, npu_id.bits.arch_minor,
		npu_id.bits.arch_rev, big_fw->fw_cnt);
	...
	while (i--) {
		...
		dev_dbg(core->dev, "Skip FW min=0x%08x, max=0x%08x\n",
			big_fw->desc[i].arch_min, big_fw->desc[i].arch_max);
	}
 ...
 * Return: 0 on success, else error code.
 */
static int firmware_load(struct ethosn_core *core, const char *firmware_name)
{
	...
	dev_dbg(core->dev,
		...
		big_fw_desc->arch_min, big_fw_desc->arch_max,
		big_fw_desc->offset, big_fw_desc->size);
...
{
	static const char *const firmware_names[] = { "ethosn.bin" };
	int i;
	int ret;

...
static int memalloc_release(struct inode *inode, struct file *filp) {
  ...
  for (i = 0; i < chunks; i++) {
    ...
    if (hlina_chunks[i].filp == filp) {
      printk(KERN_WARNING "memalloc_d: Found unfreed memory at release time!\n");
...
void __exit memalloc_d_cleanup(void) {
  if (hlina_chunks != NULL) vfree(hlina_chunks);

  unregister_chrdev(memalloc_major, DEV_NAME);
...
int __init memalloc_d_init(void) {
  int result;

  PDEBUG("module init\n");
  printk("memalloc_d: Linear Memory Allocator\n");
  printk("memalloc_d: Linear memory base = 0x%08lx\n", alloc_base);

  chunks = (alloc_size * 1024 * 1024) / CHUNK_SIZE;

  printk(KERN_INFO
         "memalloc_d: Total size %d MB; %d chunks"
         " of size %lu\n",
         alloc_size, (int)chunks, CHUNK_SIZE);

  hlina_chunks = (hlina_chunk *)vmalloc(chunks * sizeof(hlina_chunk));
  if (hlina_chunks == NULL) {
    printk(KERN_ERR "memalloc_d: cannot allocate hlina_chunks\n");
    result = -ENOMEM;
    goto err;
  }

  result = register_chrdev(memalloc_major, DEV_NAME, &memalloc_fops);
  if (result < 0) {
    PDEBUG("memalloc_d: unable to get major %d\n", memalloc_major);
    goto err;
...
static int AllocMemory(unsigned long *busaddr, unsigned int size,
  ...
  *busaddr = 0;
  ...
  if (*busaddr == 0) {
    printk("memalloc_d: Allocation FAILED: size = %d\n", size);
    return -EFAULT;
  } else {
    PDEBUG("MEMALLOC_D OK: size: %d, reserved: %d\n", size,
  ...
  for (i = 0; i < chunks; i++) {
    ...
    if (hlina_chunks[i].bus_address == busaddr + addr_transl) {
      if (hlina_chunks[i].filp == filp) {
        ...
        printk(KERN_WARNING "memalloc_d: Owner mismatch while freeing memory!\n");
      }
      break;
    }
  }
...
module_init(memalloc_d_init);
module_exit(memalloc_d_cleanup);

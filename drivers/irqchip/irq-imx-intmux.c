...
static void imx_intmux_irq_unmask(struct irq_data *d)
{
	...
	raw_spin_lock_irqsave(&intmux_data->lock, flags);
	reg = intmux_data->regs + CHANIER(idx);
	val = readl_relaxed(reg);
	val |= 1 << d->hwirq;
	writel_relaxed(val, reg);
	raw_spin_unlock_irqrestore(&intmux_data->lock, flags);
}
...
#ifdef CONFIG_IPIPE
	.irq_mask_ack	= imx_intmux_irq_mask,
#endif /* CONFIG_IPIPE */	
	.irq_unmask	= imx_intmux_irq_unmask,
	.irq_ack	= imx_intmux_irq_ack,
#ifdef CONFIG_IPIPE
	.irq_hold		= imx_intmux_irq_mask,
	.irq_release	= imx_intmux_irq_unmask,
#endif	

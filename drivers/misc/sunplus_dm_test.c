...
/*
 * Dummy Master device driver for SUNPLUS SP7021/Q645/Q654/SP7350 SoC
 *
 *  <dump help info>
 ...
 *       set WBE=1 and Write Length = 16 and op_mode = 1 / 2 / 3 / 10 (Q645/Q654/SP7350)
 *           op_mode = 1 (only write)
 *           op_mode = 2 (only read)
 *           op_mode = 3 (1 write 1 read)
 *           op_mode = 15(SP7021) 10(Q645/Q654) (1 write 3 read)
...
#if defined(Q654_DM_TEST) || defined(SP7350_DM_TEST)
...
#if defined(Q645_DM_TEST) || defined(Q654_DM_TEST) || defined(SP7350_DM_TEST)
/* For Q645/Q654 OP_MODE setting */
	"4'b0000 = IDEL",
	"4'b0001 = ONLY_WRITE",
	"4'b0010 = ONLY_READ",
	"4'b0011 = READ_after_WRITE(default)",
	"4'b0100 = WRITE and wdata equal golden value",
	"4'b0101 = READ and check rdata equal golden value",
	"4'b0110 = READ_after_WRITE and selftest with golden value",
	"4'b0111 = WRITE and write inc value",
	"4'b1000 = READ and check rdata equal inc value",
	"4'b1001 = READ_after_WRITE selftest inc value",
	"4'b1010 = 3_READ_1_WRITE",
	"4'b1011 = reserved",
	"4'b1100 = reserved",
	"4'b1101 = reserved",
	"4'b1110 = reserved",
...
u32 request_period_bw[11] = {
	#if defined(Q654_DM_TEST) || defined(SP7350_DM_TEST) /* For Q654 */
	...
	#elif defined(Q645_DM_TEST) /* For Q645 */
	0x00000078, //Target BW 10%
	0x0000003c, //Target BW 20%
	0x00000028, //Target BW 30%
	0x0000001e, //Target BW 40%
...
static int test_set(const char *val, const struct kernel_param *kp)
{
	...
	case 3:
		...
		if ((value1 == 1) && (value2 == 0) && (value3 == 0)) {
			...
			#if defined(Q645_DM_TEST) /* For Q645 */
			/*
			 * Enable Dummy Master module
			 */
			writel(0x00000000 , sp_dm_test->base0 + DM_CONTROL); /* Dummy Master 0 Enable */
			...
			pr_info("\n");
			#if defined(Q654_DM_TEST) || defined(SP7350_DM_TEST) /* For Q654 DummyMaster1 */
			for(i = 0; i < 11; i++) {
				pr_info("G337.%02d : 0x%08x(%s)\n", i, readl(sp_dm_test->base1 + i * 4),
					DummyMasterDefine[i]);
			}
			#elif defined(Q645_DM_TEST) /* For Q645 DummyMaster1 and DummyMaster2 */
			for(i = 0; i < 11; i++) {
				pr_info("G337.%02d : 0x%08x(%s)\n", i, readl(sp_dm_test->base1 + i * 4),
					DummyMasterDefine[i]);
			}
...
static const struct of_device_id sp_dm_test_of_match[] = {
	...
	{ .compatible = "sunplus,q654-dm-test" },
	{ .compatible = "sunplus,sp7350-dm-test" },
	{}
};
MODULE_DEVICE_TABLE(of, sp_dm_test_of_match);

#if 0//defined(Q645_DM_TEST) /* For Q645 DummyMaster0, DummyMaster1 and DummyMaster2 */
